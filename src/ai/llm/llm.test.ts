/**
 * Comprehensive tests for the new LLM tier
 *
 * Covers:
 *  - Router provider selection (health score, circuit breaker)
 *  - Fallback when provider returns 429 / 5xx
 *  - Cross-check (agreement, contradiction)
 *  - Redact sensitive data (sanitizer)
 *  - Integration: end-to-end with mock providers
 *  - HybridOrchestrator KB priority
 *  - AnswerCache TTL and LRU eviction
 *  - Retry helper
 */
...
// ─── unit: sanitizer ──────────────────────────────────────────────────────────
import { redactSecrets, guardInjection, sanitizePrompt, estimateTokens, truncateToTokenBudget } from './sanitizer';
...
describe('sanitizer › tokenBudget', () => {
  ...
  it('truncates to token budget', () => {
    const long = 'word '.repeat(1000); // 5000 chars ≈ 1250 tokens
    const result = truncateToTokenBudget(long, 100); // max 100 tokens = 400 chars
    expect(result.length).toBeLessThanOrEqual(450); // 400 + suffix
...
describe('crossChecker', () => {
  ...
  it('returns high confidence when two answers agree', () => {
    // Very similar answers → should agree
  ...
  it('returns synthesised hedged response on contradiction', () => {
    ...
    // Very different topics / content → low Jaccard → synthesised or low confidence
...
describe('AnswerCache', () => {
  ...
  it('returns null after TTL expiry', async () => {
    const cache = new AnswerCache(50, 10); // 50ms TTL
...
describe('ProviderMetricsTracker', () => {
  ...
  it('resets circuit after cooldown', async () => {
    const t = new ProviderMetricsTracker(2, 50); // 50ms cooldown
    t.recordFailure('prov', 100);
    t.recordFailure('prov', 100);
    expect(t.isCircuitOpen('prov')).toBe(true);
    await new Promise(r => setTimeout(r, 60));
...
import { LLMProvider, ProviderError, ProviderHealth } from './types';

/** Factory for a fully-healthy mock provider */
...
/** Mock provider that always throws a given error */
function makeFailingProvider(id: string, kind: string, statusCode?: number): LLMProvider {
  return {
    id,
    label: `Failing ${id}`,
    supportsJsonMode: false,
...
describe('LLMRouter integration', () => {
  ...
  it('returns answer from primary provider', async () => {
    ...
    const res = await router.query('What is XSS?');
  ...
  it('falls back to secondary provider on 429 rate-limit', async () => {
    ...
    const res = await router.query('What is SQL injection?');
    expect(res.providerUsed).toBe('together');
    expect(res.answer).toContain('SQL');
    expect(res.warnings.some(w => w.includes('groq'))).toBe(true);
  });
  ...
  it('falls back to secondary provider on 5xx server error', async () => {
    ...
    const res = await router.query('What is CSRF?');
    expect(res.providerUsed).toBe('together');
  });
  ...
  it('returns error response when ALL providers fail', async () => {
    ...
    const res = await router.query('What is SSRF?');
  ...
  it('cross-checks with two providers when confidence is low', async () => {
    const shortAnswer = 'XSS is bad.'; // very short → low confidence
    ...
    const router = new LLMRouter([p1, p2], metrics, {
      ...
      crossCheckThreshold: 0.99, // force cross-check for any answer
      maxRetries: 0,
      providerPriority: ['groq', 'together'],
    });
  ...
  it('serves from cache on repeated identical question', async () => {
    ...
    await router.query('What is SSRF?');
    const res2 = await router.query('What is SSRF?');
    // Second call should be from cache
    expect(res2.warnings).toContain('Served from cache');
    // generate should only have been called once
    expect(p1.generate).toHaveBeenCalledTimes(1);
  });

  it('skips provider with open circuit breaker', async () => {
    // Trip groq circuit breaker manually
    ...
    const res = await router.query('What is JWT?');
    expect(res.providerUsed).toBe('together');
    // groq.generate should NOT have been called (circuit open)
...
describe('HybridOrchestrator', () => {
  it('returns KB answer for known security topic (no LLM needed)', async () => {
    const orch = new HybridOrchestrator(null); // no LLM router
  ...
  it('falls back gracefully when LLM router is null and KB has no match', async () => {
    const orch = new HybridOrchestrator(null);
    const res  = await orch.orchestrate({ question: 'thời tiết hôm nay thế nào' }); // out-of-scope
    expect(res).toHaveProperty('answer');
  ...
  it('uses LLM when KB has no match and router is available', async () => {
    ...
    // Use a question that KB likely won't answer deeply
    const res = await orch.orchestrate({ question: 'OAuth 2.0 implicit flow security risks' });
    // Either from KB or LLM — just ensure it has an answer
