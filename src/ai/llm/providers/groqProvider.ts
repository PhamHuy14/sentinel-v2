 ...
 * Model: llama3-8b-8192  (fast, generous free quota)
...
export class GroqProvider implements LLMProvider {
  readonly id = 'groq';
  readonly label = 'Groq (Llama-3 8B)';
  ...
  constructor(metrics: ProviderMetricsTracker) {
    // Read from Vite env (browser/renderer) — never hardcode
    this.apiKey = (import.meta as unknown as Record<string, Record<string, string>>).env?.VITE_GROQ_API_KEY ?? '';
  ...
  async estimateCostOrQuota(): Promise<number> {
    // Groq free tier: ~14,400 req/day for llama3-8b — return remaining as weight
